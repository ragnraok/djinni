...
//      javaAnnotationHeader.foreach(w.wl)
      ...
      var classDefPrefix = ""
      if (i.ext.cpp) {
        classDefPrefix = "abstract class "
      } else if (i.ext.java) {
        classDefPrefix = "interface "
      }
      
      w.w(s"${javaClassAccessModifierString}$classDefPrefix$javaClass$typeParamList$extendLiter").braced {
        val skipFirst = SkipFirst()
...
//          marshal.nullityAnnotation(m.ret).foreach(w.wl)
          if (i.ext.cpp) {
            ...
            w.wl("public " + ret + " " + idJava.method(m.ident) + params.mkString("(", ", ", ")") + throwException + ";")
          }
        }
        for (m <- i.methods if m.static) {
          skipFirst { w.wl }
...
//          val nonnullAnnotation = javaNonnullAnnotation.map(_ + " ").getOrElse("")
          w.w("public int compareTo(self other) ").braced {
            w.wl("int tempResult;")
            for (f <- r.fields) {
              f.ty.resolved.base match {
